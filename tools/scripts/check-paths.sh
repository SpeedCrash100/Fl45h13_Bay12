...
exactly 150 "to_world uses" '\sto_world\('
...
exactly 744 "<< uses" '(?<!<)<<(?!<)' -P
exactly 535 "incorrect indentations" '^( {4,})' -P
exactly 33 "text2path uses" 'text2path'
exactly 367 "update_icon() override" '/update_icon\((.*)\)'  -P
exactly 2 "goto use" 'goto '
exactly 860 "spawn uses" 'spawn\s*\(\s*(-\s*)?\d*\s*\)' -P
exactly 513 "tag uses" '\stag = ' -P '**/*.dmm'
exactly 299 "/global/ or /static/ vars defined" '/(global|static)/' -P
# With the potential exception of << if you increase any of these numbers you're probably doing it wrong
#
#broken_files=0
